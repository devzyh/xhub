...
$(function () {
     ...
     * 获取运行时间
     */
    var runDate = function () {
        var r = $result.val().trim().replace(/，/g, ',')
            .replace(/\s+/g, ' ').toLocaleUpperCase();
        $.post("/cron/run_time", {"cron": r}, function (data) {
            $("#runTime").empty();
            for (let i in data) {
                $("#runTime").append("<dd>" + data[i] + "</dd>");
            }
        });
    }
    ...
    TimeObject.forEach(function (v) {
        ...
        if ($currChosen) {
            for (; min <= max; min++) {
                var option = "<div class='block'><input type='checkbox' class='cbx' name='"
                    + idAssigned + "' value='" + min + "'/>" + min + "</div>";
